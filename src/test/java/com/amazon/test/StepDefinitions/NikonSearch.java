...
public class NikonSearch {
  ...
  {
	  int elementCtr=0;
	  WebElement resultElt=null;
	  List<WebElement> resultTable=driver.findElements(By.cssSelector("h2[class='a-size-medium s-inline  s-access-title  a-text-normal']"));
	  System.out.println(resultTable.size());
	  for(WebElement elt:resultTable)
	  {
		  elementCtr++;
		  if(elementCtr==2)
		  {
			 //Get Parent
			  resultElt=elt.findElement(By.xpath(".."));
			  System.out.println(resultElt.getAttribute("href"));
			  driver.get(resultElt.getAttribute("href"));
			  break;
		  }
	  }
